...
local_model_name = "deepseek-ai/deepseek-coder-7b-instruct-v1.5"
...
client_gpt = AutoModelForCausalLM.from_pretrained(local_model_name, torch_dtype=torch.bfloat16, trust_remote_code=True)
if torch.cuda.is_available():
        client_gpt = client_gpt.cuda()
client_gpt.generation_config = GenerationConfig.from_pretrained(local_model_name)
client_gpt.generation_config.pad_token_id = client_gpt.generation_config.eos_token_id
...
for current_iteration in range(NUM_ITERATIONS):
    ...
    for i in range(1, number_of_questions+1):
        ...
        after_template_messages = local_tokenizer.apply_chat_template(messages, add_generation_prompt=True, return_tensors="pt")
        if not isinstance(after_template_messages, dict):
            after_template_messages = {"input_ids": after_template_messages, "attention_mask": torch.ones_like(after_template_messages)}
        else:
            if "attention_mask" not in after_template_messages:
                after_template_messages["attention_mask"] = torch.ones_like(after_template_messages["input_ids"])
        after_template_messages = {k: v.to(client_gpt.device) for k, v in after_template_messages.items()}
        
        
        response = client_gpt.generate(
        input_ids=after_template_messages["input_ids"],
        attention_mask=after_template_messages["attention_mask"],
        max_new_tokens=512,
        do_sample=False,
        num_return_sequences=1,
        eos_token_id=local_tokenizer.eos_token_id)

        action_sequence = local_tokenizer.decode(response[0][len(after_template_messages["input_ids"][0]):], skip_special_tokens=True)
        
        ## FIXME Qihan: for deekseep, we need to extract the hint from the action sequence
    ...
    if (current_iteration+ 1) % 5 == 0:
        send_email("llmqo LOCAL Experiment 1", f"The experiment of IMDB finished! The iteration is {current_iteration + 1}", "")
