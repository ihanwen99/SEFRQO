...
DATABASE_NAME_2 = "imdbload_after2000"
...
print(f"Database name 2: {DATABASE_NAME_2}")
...
TEST_SQL_DIR = "/home/your_username/LLM4QO/sql/imdb_assorted_5"
...
if f"{SQL_INIT_COLLECTION_NAME}" not in collections:
    ...
    for entry in data:
        ...
        if match:
            ...
            execution_time = float(match.group(2).strip())  
            plan_comment = match.group(3).strip()          
            sql_text = match.group(4).strip()              
...
def execute_sql(sql_query, print_dummy_join=False, only_order=False,used_database=DATABASE_NAME):
    """
...
USE_DATABASE_2 = [5,6,7,8,9,15,16,17,18,19,25,26,27,28,29,35,36,37,38,39,45,46,47,48,49,55,56,57,58,59,65,66,67,68,69,75,76,77,78,79,85,86,87,88,89,95,96,97,98,99]
...
for current_iteration in range(NUM_ITERATIONS):
    ...
    if current_iteration in USE_DATABASE_2:
    ...
    for i in range(1, number_of_questions+1):
        ...
        if USE_REFERENCE:
            if current_iteration == 1:
                ...
                # Safety check: ensure we have reference queries available
                if not milvus_res_envs or not milvus_res:
                    print(f"Warning: No reference queries available for question {keys[i-1]}")
                    num_available_refs = 0
                else:
                    num_available_refs = min(NUM_REFERENCE_QUERIES, len(milvus_res_envs), len(milvus_res))
                    if num_available_refs < NUM_REFERENCE_QUERIES:
                        print(f"Warning: Only {num_available_refs} reference queries available (requested {NUM_REFERENCE_QUERIES})")
                
                for j in range(num_available_refs):
                    try:
                        combined_query += milvus_res_envs[j] + "\nThe optimal hint:\n" + milvus_res[j] + "\n"
                    except (IndexError, TypeError) as e:
                        print(f"Error processing reference query {j}: {str(e)}")
                        break
        ...
        exec_time, real_hint = execute_sql(sql_query_hinted, print_dummy_join=False, only_order=ONLY_ORDER,used_database=CONNECTION_INFO["database"])
        print("\n")
    ...
    if (current_iteration+ 1) % 5 == 0:
        send_email("llmqo LOCAL Experiment test1", f"The experiment of IMDB finished! The iteration is {current_iteration + 1}", "")
