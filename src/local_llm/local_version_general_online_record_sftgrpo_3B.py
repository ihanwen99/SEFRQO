...
local_model_name = "/home/user_name/LLM4QO/Apr12-3B-GRPO/checkpoint-500"
...
if f"{SQL_INIT_COLLECTION_NAME}" not in collections:
    ...
    for entry in data:
        ...
        if match:
            file_id = match.group(1).strip()              # the file id, e.g., "01.sql"
            execution_time = float(match.group(2).strip())  # the original execution plan time (unit: ms), e.g., 3034.749
            plan_comment = match.group(3).strip()           # the execution plan comment block, e.g., /*+ ... */
            sql_text = match.group(4).strip()               # the SQL statement part
