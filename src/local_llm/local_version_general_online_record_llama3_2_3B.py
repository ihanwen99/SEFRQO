...
# use bitsandbytes to load 4bit quantized model
...
if f"{SQL_INIT_COLLECTION_NAME}" not in collections:
    ...
    # read SQL query content from file, assuming each entry is separated by an empty line
...
for current_iteration in range(NUM_ITERATIONS):
    ...
    if (current_iteration+ 1) % 5 == 0:
        send_email("llmqo LOCAL Experiment ", f"The experiment of IMDB LLAMA4 finished! The iteration is {current_iteration + 1}", "")
