...
# And the solution is:
 ...
 Pick table ci, with a label "IndexScan".
 Join table n and ci as (n ci), with a label "NestLoop".
 Check remaining tables and subtrees: it1 it2 mi mi_idx t (n ci).
 ...
 Join table t and subtree (n ci) as (t (n ci)), with a label "HashJoin".
 Check remaining tables and subtrees: it1 it2 mi mi_idx (t (n ci)).
 Decide not to divide the candidate table set into two subset, and tackle each of them recursively.
 Pick table mi, with a label "IndexScan".
 Join subtree (t (n ci)) with table mi as ((t (n ci)) mi), with a label "NestLoop".
 Check remaining tables and subtrees: it1 it2 mi_idx ((t (n ci)) mi).
 Decide not to divide the candidate table set into two subset, and tackle each of them recursively.
 Pick table it1, with a label "SeqScan".
 Join subtree ((t (n ci)) mi) with table it1 as (((t (n ci)) mi) it1), with a label "MergeJoin".
 Check remaining tables and subtrees: it2 mi_idx (((t (n ci)) mi) it1).
 Decide not to divide the candidate table set into two subset, and tackle each of them recursively.
 Pick table mi_idx, with a label "SeqScan".
 Join subtree (((t (n ci)) mi) it1) with table mi_idx as ((((t (n ci)) mi) it1) mi_idx), with a label "HashJoin".
 Check remaining tables and subtrees: it2 ((((t (n ci)) mi) it1) mi_idx).
 Decide not to divide the candidate table set into two subset, and tackle each of them recursively.
 Pick table it2, with a label "IndexScan".
 Join subtree ((((t (n ci)) mi) it1) mi_idx) with table it2 as (((((t (n ci)) mi) it1) mi_idx) it2), with a label "HashJoin".
 Check remaining tables and subtrees: ((((t (n ci)) mi) it1) mi_idx).
